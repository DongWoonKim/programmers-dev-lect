...
fun main() {
    ...
    // 문자열에서의 + 는 '이어 붙이기'가 된다. 같은 기호라도 자료형에 따라 동작이 다르다.
    ...
    num /= 4            // num = num / 4  (정수 나눗셈이라 7.5 가 아니라 7)
    println("num /= 4      -> $num")     // 7

    num %= 3            // num = num % 3
    println("num %= 3      -> $num")     // 1

    // val 은 값을 바꿀 수 없으므로 복합 대입 연산자도 쓸 수 없다.
    val fixed = 10
    ...
    count++             // count = count + 1 과 같다
    println("count++ 후: $count")        // 11

    count--             // count = count - 1 과 같다
    println("count-- 후: $count")        // 10

    // 3-1. 전위(++변수) : 값을 먼저 1 늘리고, 그 늘어난 값을 사용한다.
    var x = 10
    println("++x 의 결과: ${++x}")        // 11  (먼저 11로 만들고 사용)
    println("사용 후 x  : $x")            // 11

    // 3-2. 후위(변수++) : 지금 값을 먼저 사용하고, 그 다음에 1 늘린다.
    var y = 10
    println("y++ 의 결과: ${y++}")        // 10  (10을 먼저 쓰고 나서 11로 바뀜)
    println("사용 후 y  : $y")            // 11

    // 결과만 놓고 보면 x 와 y 는 똑같이 11 이지만,
    // '그 줄에서 사용된 값'이 11 이냐 10 이냐가 다르다. 이것이 전위와 후위의 차이다.
    ...
    // 비교 결과는 Boolean 이므로 변수에 담아둘 수 있다.
    val isBigger: Boolean = q > p
    println("q 가 더 큰가? $isBigger")     // true

    // 문자열도 == 로 내용을 비교한다. (자바의 equals() 에 해당)
    println("kotlin" == "kotlin")        // true
    println("kotlin" == "Kotlin")        // false  대소문자를 구분한다

    // 참고) == 는 값 비교, === 는 같은 객체인지(주소) 비교. B_basic_type_2 에서 다뤘다.
    ...
    // 비교 연산자와 조합해서 조건을 여러 개 묶는 것이 실제 사용법이다.
    val age = 25
    val hasTicket = true
    ...
    // && 는 왼쪽이 false 면 오른쪽을 아예 계산하지 않는다. (이미 false 로 확정)
    // || 는 왼쪽이 true 면 오른쪽을 아예 계산하지 않는다. (이미 true 로 확정)
    var check = 0

    val r1 = false && (check++ > 0)      // 왼쪽이 false -> 오른쪽 실행 안 됨
    println("r1 = $r1, check = $check")  // r1 = false, check = 0  (증가하지 않았다!)

    val r2 = true || (check++ > 0)       // 왼쪽이 true -> 오른쪽 실행 안 됨
    println("r2 = $r2, check = $check")  // r2 = true, check = 0

    val r3 = true && (check++ > -1)      // 왼쪽이 true -> 오른쪽까지 실행됨
    println("r3 = $r3, check = $check")  // r3 = true, check = 1  (증가했다!)
    ...
    println((2 + 3) * 4)                 // 20   괄호로 순서를 바꿀 수 있다
